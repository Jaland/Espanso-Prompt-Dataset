...
def generate_main_config(prompts_dir, output_file):
    ...
    # Check for global variables file
    global_vars_file = os.path.join(prompts_dir, 'global-variables.yaml')
    if os.path.exists(global_vars_file):
        print(f"Processing global variables: {global_vars_file}")
        global_content = load_yaml_file(global_vars_file)
        if global_content and 'global_vars' in global_content:
            main_config['global_vars'] = global_content['global_vars']
            print(f"Added {len(global_content['global_vars'])} global variables")
        
        # Also process matches from global variables file
        if global_content and 'matches' in global_content:
            main_config['matches'].extend(global_content['matches'])
...
# Generated from {len(prompt_files)} prompt files:"""
    ...
    # Process each prompt file (excluding global-variables.yaml which was already processed)
    for file_path in prompt_files:
        # Skip global variables file since we already processed it
        if os.path.basename(file_path) == 'global-variables.yaml':
            continue
